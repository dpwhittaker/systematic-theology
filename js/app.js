...
async function loadHandout(path) {
    try {
        ...
        if (path.startsWith('storyboards/') && path.endsWith('.md') && window.StoryboardEditor) {
            ...
            window.StoryboardEditor.render(blocks, path, els.cardBody);
            return;
        }
        ...
        // Storyboard-specific transforms (fallback when editor JS not loaded)
        if (path.startsWith('storyboards/')) {
            html = html.replace(
                /<blockquote>(?:\[!slide\]|<a[^>]*>!slide<\/a>|\[!slide])<br>([\s\S]*?)<\/blockquote>/g,
                (match, content) => '<div class="slide-callout"><span class="slide-label">Slide</span><br>' + content + '</div>'
            );
            const speakers = { WRIGHT: 'wright', CREED: 'creed', CARRIE: 'carrie', HOST: 'host' };
            for (const [id, cls] of Object.entries(speakers)) {
                html = html.replace(new RegExp(`<strong>${id}:<\\/strong>`, 'g'), `<strong class="speaker-${cls}">${id}:</strong>`);
            }
            ...
            html = html.replace(/<img src="([^"]*\.mp3)" alt="audio"[^>]*>/g,
                (m, src) => `<div class="section-audio"><audio controls preload="none" src="${src}"></audio></div>`);
